...
if [ "$WANT_BUILD_DEPS" = 'Y' ]; then
  ...
  debian_pkg_list_tools="python3 libsodium-dev systemd libncursesw5 gnupg aptitude tmux jq libtool autoconf secure-delete iproute2 bc tcptraceroute dialog sqlite automake sqlite3 bsdmainutils libusb-1.0-0-dev libudev-dev"
  rhel_pkg_list_tools="python3 libsodium-devel coreutils ncurses-libs ncurses-compat-libs systemd tmux jq gnupg libtool autoconf srm iproute bc tcptraceroute dialog sqlite util-linux xz libusb-devel"
